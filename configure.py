...
# echo "configure replace file:$PWD/template.j2" | FastCli -p15	
def apply_config(config):
	with tempfile.NamedTemporaryFile() as config_file:
		config_file.write(config.encode())
		try:
			return run(f"echo 'configure replace file:{config_file.name}' | FastCli -p15", shell=True, check=True, stdout=PIPE, stderr=PIPE).stdout
